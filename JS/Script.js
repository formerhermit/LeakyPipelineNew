/**
 * The Leaky Pipeline — game logic.
 *
 * Vanilla JS, no jQuery / jQuery UI. Native <dialog> elements handle the
 * welcome, scenario, feedback and results modals. All question, feedback,
 * results and action copy below is unchanged from the original build.
 */
( function () {
	'use strict';

	var pipeStateEnum = { start: 'start', fixed: 'fixed', leaky: 'leaky' };

	var pipeStateArray = [
		[ pipeStateEnum.start, pipeStateEnum.start ],
		[ pipeStateEnum.start, pipeStateEnum.start ]
	];

	var icons = {
		School: '<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M24 6 4 15l20 9 20-9Z"/><path d="M12 20v11c0 3 5.4 7 12 7s12-4 12-7V20"/><path d="M44 15v14"/></svg>',
		University: '<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M6 20 24 8l18 12"/><path d="M10 20v18h28V20"/><path d="M20 38v-9h8v9"/><path d="M4 41h40"/></svg>',
		Work: '<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="6" y="16" width="36" height="24" rx="3"/><path d="M17 16v-4a3 3 0 0 1 3-3h8a3 3 0 0 1 3 3v4"/><path d="M6 27h36"/><path d="M21 27v4h6v-4"/></svg>'
	};

	var imageArray = [ [ icons.School, icons.University ], [ icons.Work, icons.Work ] ];

	// Which static elbow joints sit against each pipe section, so a fixed
	// section can visibly send its flow on into the connected joint(s).
	var jointMap = {
		'0,0': [ 'lp-joint-top' ],
		'0,1': [ 'lp-joint-top', 'lp-joint-right' ],
		'1,1': [ 'lp-joint-right', 'lp-joint-bottom' ],
		'1,0': [ 'lp-joint-bottom' ]
	};
	...
	var titles = [ 'Plumbing Student', 'Apprentice Plumber', 'Wrench Wrangler', 'Pipe Warrior', 'Plumbing Overlord' ];

	var results = [
		'You have scored 0 out of 4. Thank you for taking the time to learn more about the leaky pipeline and the issues which affect women in their STEM career journey! If this is all new information to you, please don’t feel bad – you’re certainly not alone, hence the reason for this game being created.',
	...
	var actions = [
		'The STEM skills shortage is costing the British economy £63 billion a year <a href="http://www.wearethecity.com/girls-perform-better-boys-choose-subject/" target="_blank"> (WeAreTheCity 2016)</a>, is your organisation missing out on talent?',
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		var welcomeDialog = document.getElementById( 'lp-welcome-dialog' );
		var scenarioDialog = document.getElementById( 'lp-scenario-dialog' );
		var feedbackDialog = document.getElementById( 'lp-feedback-dialog' );
		var resultsDialog = document.getElementById( 'lp-results-dialog' );

		if ( ! welcomeDialog || ! scenarioDialog || ! feedbackDialog || ! resultsDialog ) {
			return; // Not on the game page.
		}

		var pipes = document.querySelectorAll( '.lp-pipe' );
		var scenarioIcon = document.getElementById( 'lp-scenario-icon' );
		var scenarioQuestion = document.getElementById( 'lp-scenario-question' );
		var choiceInputs = [
			document.getElementById( 'lp-choice-1' ),
			document.getElementById( 'lp-choice-2' ),
			document.getElementById( 'lp-choice-3' )
		];
		var choiceLabels = [
			document.getElementById( 'lp-choice-label-1' ),
			document.getElementById( 'lp-choice-label-2' ),
			document.getElementById( 'lp-choice-label-3' )
		];
		var scenarioSubmit = document.getElementById( 'lp-scenario-submit' );
		var feedbackAnswer = document.getElementById( 'lp-feedback-answer' );
		var feedbackText = document.getElementById( 'lp-feedback-text' );
		var feedbackContinue = document.getElementById( 'lp-feedback-continue' );
		var resultsTitle = document.getElementById( 'lp-results-title' );
		var resultsText = document.getElementById( 'lp-results-text' );
		var resultsAction = document.getElementById( 'lp-results-action' );
		var welcomeStart = document.getElementById( 'lp-welcome-start' );
		var resultsClose = document.getElementById( 'lp-results-close' );
		...
		pipes.forEach( function ( pipe ) {
			pipe.addEventListener( 'click', function () {
				var row = Number( pipe.dataset.row );
				var col = Number( pipe.dataset.col );

				if ( pipeStateArray[ row ][ col ] !== pipeStateEnum.start ) {
					return;
				}
				...
				scenarioIcon.innerHTML = imageArray[ row ][ col ];
				scenarioQuestion.innerHTML = question.QUESTION;
				choiceLabels[ 0 ].innerHTML = question.CHOICE1;
				choiceLabels[ 1 ].innerHTML = question.CHOICE2;
				choiceLabels[ 2 ].innerHTML = question.CHOICE3;
				choiceInputs[ 0 ].checked = true;

				scenarioDialog.showModal();
			} );
		} );

		scenarioSubmit.addEventListener( 'click', function () {
			var question = questions[ currentRow ][ currentCol ];
			var checked = document.querySelector( 'input[name="lp-radchoice"]:checked' ).value;
			var pipe = document.querySelector( '.lp-pipe[data-row="' + currentRow + '"][data-col="' + currentCol + '"]' );

			scenarioDialog.close();

			if ( Number( checked ) === question.CORRECT ) {
				feedbackAnswer.textContent = question.RIGHT;
				feedbackAnswer.className = 'lp-feedback__answer lp-feedback__answer--right';
				feedbackText.innerHTML = question.FEEDBACKG;
				pipe.classList.add( 'lp-pipe--fixed' );
				pipeStateArray[ currentRow ][ currentCol ] = pipeStateEnum.fixed;
				pipe.disabled = true;
				( jointMap[ currentRow + ',' + currentCol ] || [] ).forEach( function ( id ) {
					var joint = document.getElementById( id );
					if ( joint ) {
						joint.classList.add( 'is-flowing' );
					}
				} );
				burstFix( pipe );
				playSound( 'Cheer' );
			} else {
				feedbackAnswer.textContent = question.WRONG;
				feedbackAnswer.className = 'lp-feedback__answer lp-feedback__answer--wrong';
				feedbackText.innerHTML = question.FEEDBACKP;
				pipe.classList.add( 'lp-pipe--leaky' );
				pipeStateArray[ currentRow ][ currentCol ] = pipeStateEnum.leaky;
				pipe.disabled = true;
				burstLeak( pipe );
				playSound( 'Splash' );
			}
		...
		feedbackContinue.addEventListener( 'click', function () {
			...
			if ( countInArray( pipeStateEnum.start ) === 0 ) {
				var score = countInArray( pipeStateEnum.fixed );

				resultsDialog.querySelectorAll( '.lp-badge' ).forEach( function ( badge ) {
					badge.classList.add( 'hidden' );
				} );
				resultsDialog.querySelector( '.lp-badge--' + score ).classList.remove( 'hidden' );

				resultsTitle.textContent = titles[ score ];
				resultsText.textContent = results[ score ];
				resultsAction.innerHTML = actions[ score ];
		...
		function countInArray( item ) {
			var count = 0;
			pipeStateArray.forEach( function ( row ) {
	...
	// -------------------------------------------------------------------
	document.addEventListener( 'DOMContentLoaded', function () {
		var toggle = document.querySelector( '.lp-nav__toggle' );
	...
	// -------------------------------------------------------------------
	// Full-viewport effects: a short leak splash, a fix spark burst, and a
	// bigger confetti burst for the final results reveal.
	// -------------------------------------------------------------------
	function spawnLayer() {
		var layer = document.createElement( 'div' );
		layer.className = 'lp-fx-layer';
	...
	function burstLeak( fromEl ) {
		...
		for ( var i = 0; i < 14; i++ ) {
			...
			drop.className = 'lp-fx-drop';
			var size = 8 + Math.random() * 12;
			drop.style.left = ( rect.left + rect.width * Math.random() ) + 'px';
			drop.style.width = size + 'px';
			drop.style.height = size * 1.3 + 'px';
			drop.style.setProperty( '--fall', ( 200 + Math.random() * 260 ) + 'px' );
	...
	function burstWin() {
		...
		spawnSparks( window.innerWidth / 2, window.innerHeight / 3, 60 );
	}

	function spawnSparks( x, y, count ) {
		var layer = spawnLayer();
		var colors = [ '#8B5CF6', '#2DD4BF', '#FBBF24', '#FB7185', '#F5F3FF' ];

		for ( var i = 0; i < count; i++ ) {
			var spark = document.createElement( 'div' );
			spark.className = 'lp-fx-spark';
			...
			spark.style.setProperty( '--tx', ( -160 + Math.random() * 320 ) + 'px' );
			spark.style.setProperty( '--ty', ( -160 + Math.random() * 260 ) + 'px' );
